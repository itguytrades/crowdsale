...
describe('Crowdsale', () => {
  ...
  beforeEach(async () => {
      ...
      token = await Token.deploy('CGDev Token', 'DAPP', '1000000')
      ...
      crowdsale = await Crowdsale.deploy(token.address, ether(1), maxSupply, startTime, endTime, minPurchase, maxPurchase)
  ...
  describe('Updating Whitelist', () => {
    ...
    describe('Success', () => {
      ...
      it('checks for investor address in whitelist', async () => {
            expect(await crowdsale.whitelist(user1.address)).to.be.true;
      })
    })
    describe('Failure', () => {
      it('only owner can update whitelist', async () => {
      await expect (crowdsale.connect(user1).addToWhitelist(user2.address, true)).to.be.reverted;
      })
    })
 })
